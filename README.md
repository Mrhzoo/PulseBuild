...
**S0–S32** on `main`. Public register is closed in production. Create a pilot Owner with `python -m scripts.create_pilot_owner` — see [docs/S31-publish-ready.md](docs/S31-publish-ready.md). Briefing time is per tenant (timezone + HH:MM).
...
Digest cron (every 15 min): `cd backend && python -m scripts.send_morning_digests`
